...
public class UserController {
    ...
    public Response<User> login(@RequestBody User user) {
        ...
        if (requestUsername == null) {
            return ResponseUtils.fail(ResponseStatus.BAD_REQUEST, "用户名不能为空");
        }
        User registeredUser = userService.findByUsername(requestUsername).orElse(null);
        if (registeredUser == null) {
            return ResponseUtils.fail(ResponseStatus.BAD_REQUEST, "用户不存在");
        }
        if (!registeredUser.getPassword().equals(user.getPassword())) {
            return ResponseUtils.fail(ResponseStatus.BAD_REQUEST, "密码不正确");
        }
